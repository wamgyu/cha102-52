...
public class DeptJDBCDAO implements DeptDAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO dept3 (dname,loc) VALUES (?, ?)";
	private static final String GET_ALL_STMT = "SELECT deptno , dname, loc FROM dept3";
	private static final String GET_ONE_STMT = "SELECT deptno , dname, loc FROM dept3 where deptno = ?";
	private static final String GET_Emps_ByDeptno_STMT = "SELECT empno,ename,job,hiredate,sal,comm,deptno FROM emp3 where deptno = ? order by empno";
	
	private static final String DELETE_EMPs = "DELETE FROM emp3 where deptno = ?";
	private static final String DELETE_DEPT = "DELETE FROM dept3 where deptno = ?";	
	
	private static final String UPDATE = "UPDATE dept3 set dname=?, loc=? where deptno = ?";
...
//		dao.delete(3);
		...
		// 查詢
//		DeptVO deptVO3 = dao.findByPrimaryKey(1);
...
//		System.out.println("---------------------");
		...
		// 查詢某部門的員工
		Set<EmpVO> set = dao.getEmpsByDeptno(1);
		for (EmpVO aEmp : set) {
